    ...
    $('#create-new-account').on('click', event => {
      ...
      if (
        !$('#create-username')
          .val()
          .trim()
          .match(/^[a-zA-Z0-9 _-]{1,15}$/)
      ) {
        errors = true
        errorArray.push(
          "'Username' field must be from 1 to 15 characters with no special characters!"
        )
      }
      if (
        !$('#create-password')
        ...
        errors = true
        errorArray.push(
          "'Password' field must be from 1 to 15 characters with no special characters!"
        )
      }
      if (
        !$('#create-email')
